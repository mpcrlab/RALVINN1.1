...
class roverSim():
    ...
    def test(self, pr=False):
        ...
        steps = 0
        temp_state = [0, 0, 0]
        while (not terminal) and steps < 20:
            steps += 1
            action = self.agent.choose_action(temp_state, 0)
            action = self.actions[action]
            if action == "left":
                temp_state = self.deserialize(self.leftTransitions[self.serialize(temp_state)])
            elif action == "right":
                temp_state = self.deserialize(self.rightTransitions[self.serialize(temp_state)])
                reward, terminal = valueFunction(temp_state)
            if pr:
                print temp_state
        print steps
